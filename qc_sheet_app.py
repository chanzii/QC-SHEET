...
if st.button("🚀 QC시트 생성"):
    ...
    # ----------- 1. 스펙 워크시트 찾기 -----------
    wb_spec = load_workbook(spec_path, data_only=True, read_only=True)  # read_only=True 적용
    ...
    if selected_size not in size_idx_map:
        ...
        st.stop()
    size_col_zero = size_idx_map[selected_size]
    ...
    wb_tpl.save(buffer)
    buffer.seek(0)
    st.download_button("📥 QC시트 다운로드", buffer.getvalue(), file_name=out_name)
